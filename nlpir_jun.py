...
This warpper is for python 2.x, Python 3.x users should notice:
read the notice at the last part of the code carefully and change the configurations starts with ###
'''
from ctypes import *
...
libFile = './NLPIR64.dll'
# libFile = 'NLPIR32.dll' # for python 32-bit environment
...
# for nltk
def tag(tokens): # tokens = ['John', 'saw', 'Mary'] -> [(John, NN),(saw, v),..]
##    sent = ''.join(tokens)
##    sent = sent.encode('UTF8')
##    return list(Seg(sent))
    tokens = [w.encode('UTF8') for w in tokens]
    result = []
    for word in tokens:
        result.extend(list(Seg(word)))
##    result = [(w,t.upper()) for (w,t) in result]
    return result

def to_byte(sent):
    return bytes(sent, 'UTF8')

def word_segmentation(sent):
    gen = Seg(to_byte(sent))
    return [w for w,t in gen]

def pos_tagging(sent):
    gen = Seg(to_byte(sent))
    return [(w,t) for (w,t) in gen]

def junjun():
    '''
read a text file and tag POS, and write it into a file.
'''
    # choose a text file.
    ...
    # read the file.
    file_handle = open(fr, encoding = enc)

    # choose a text file to write.
    ft = filedialog.asksaveasfilename()
    ft = open(ft, 'w', encoding = 'UTF8')

    # Pos tagging.
    for sent in file_handle:
        tagging_result = pos_tagging(sent)
        tagging_result = [ '/'.join(wt) for wt in tagging_result]
        ft.write(' '.join(tagging_result)+'\n')
...
def detect_encoding(file_handle):
    with open(file_handle, 'rb') as handle:
        print('detecting encoding...')
        detection = chardet.detect(handle.read())
        print(detection)
...
def tagprocessing(paragraph):
    sents = lambda x: [x if x != "" else '\n' for x in x.split('\n') ]
    for sent in sents(paragraph):
        tagging_result = pos_tagging(sent)
        tagging_result = ['/'.join(wt) for wt in tagging_result]
        yield ' '.join(tagging_result)

def segprocessing(paragraph):
    sents = lambda x: [x if x != "" else '\n' for x in x.split('\n') ]
    for sent in sents(paragraph):
        seg_result = word_segmentation(sent)
        yield ' '.join(seg_result)
...
##        print(s)
##        # Notice:  In Windows command line window (CMD), Chinese tokens might be shown as unreadable characters when printed,,
##        # this is just because Windows CMD cannot show them correctly, yet the content is actually correctly.
##        # You are suggested to use Linux or IDE like Pycharm, or write the content to a text file to read them.
##        # If you have to print strings in CMD under Python 2.x, refer to this page: http://apoo.bokee.com/7028948.html
